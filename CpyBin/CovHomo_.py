...
parser.add_argument('-n', '--nrowref', type=int, 
                    help='Calculate the reference signal on the nrowref first rows', default=10)
parser.add_argument('-b', '--reflb', type=float, 
                    help='Line broadening of reference diagonal peaks', default=0)
parser.add_argument('-w', '--refW', type=float, 
                    help='Weight of reference diagonal peaks', default=5)
parser.add_argument('-p0', '--refPh0', type=float, 
                    help='Zero order phase of reference diagonal peaks', default=0)
parser.add_argument('-p1', '--refPh1', type=float, 
                    help='First order phase of reference diagonal peaks', default=0)
parser.add_argument('-s', '--slope', type=float, 
                    help='slope of diagonal peaks : 1 for SQ-SQ or 2 for DQ-SQ', default=1)
...
DWf1 = 1e-6*dat.readacqpar("INF 1", dimension=1)
#print(DWf1)
TDf1 = dat.readacqpar("TD", dimension=2)
# if states/states-ttpi then TDf1 must be even else remove one row
if MODEflag == 4 or MODEflag == 5 :
	if TDf1%2 == 1 : TDf1 -= 1
#D0 = dat.readacqpar("D 0", dimension=1)
si2 = dat.readprocpar("FTSIZE", dimension=1)
if (si2 == 0):
	print('F2 fourier transform not performed yet : exiting...')
	sys.exit()
swh2 = dat.readacqpar("SW_h", dimension=1)
HzpPTf2 = swh2/si2
sistart = si2//2-dat.readprocpar("STSR", dimension=1)
...
NUSlist = []
print("NUSFLAG is %d" % (NUSflag))
if NUSflag == 2:
	NUSfile = dat.returnacqpath()+"nuslist"
	index_list = open(NUSfile, "r").read().strip("\n").split()
# la nuslist a des index de 0 à td1//2-1
	NUSlist = [(int(index_list[i])) for i in range(td1//2)]
else :
	NUSlist = range(td1//2)
...
#testREF(dat.readprocpar("SI", dimension=2))
