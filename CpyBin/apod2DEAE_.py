...
# shape is now (2,td1/2,td2c) with hypercomplex phases in first dimension

# lest's separate Echo and antiecho : E=Sx+iSy and AE=Sx-iSy
# depending on phase cycling Sx/Sy may be exchanged
exchSxSy=False
if exchSxSy :
    E=numpy.empty_like(summed)
    E=summed[0]-1j*summed[1]
    AE=numpy.empty_like(summed)
    AE=summed[0]+1j*summed[1]
else:
    E=numpy.empty_like(summed)
    E=summed[0]+1j*summed[1]
    AE=numpy.empty_like(summed)
...
#2D array (td1/2,td2c) representing the time t0 for gaussian apodization along +s (Echo :T0gp) and -s (Antiecho : T0gm)
...
# Rebuild Sx and Sy (Sx=1/2(E+AE), Sy=-i/2*(E-AE))
summed[0]=0.5*(E+AE)
summed[1]=-0.5*1j*(E-AE)
...
dat.writeprocpar("LB",str(args.lb),status=True,dimension=2)
dat.writeprocpar("WDW","2",status=True,dimension=1)
dat.writeprocpar("WDW","1",status=True,dimension=2)
