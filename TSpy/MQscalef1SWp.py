...
def scalef1sheared(F2toF1=False, exptype='3QMAS', dataset=None):
    ...
    swp = dtst.readprocpar("SW_p",dimension=2,status=True)
...
#    sfo += folding_times*swh/1e6 
    ...
    hzppt = swh/si
    swp = stsi*hzppt
    STSRfreq = sfo+swh/2e6-stsr*hzppt/1e6 # freq of STSR point in MHz
    STSRppm =  (STSRfreq - sf)/sf*1e6 # position fo STSR in ppm
    offset = STSRppm
    MSG(str(offset))
    dtst.writeprocpar("SW_p", swp, dimension=2, status=True)
    dtst.writeprocpar("OFFSET", STSRppm, dimension=2, status=True)
